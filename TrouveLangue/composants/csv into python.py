pourcentagesLettres = {'Lettres': ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z', 'à', 'â', 'á', 'å', 'ä', 'ã', 'a', 'æ', 'œ', 'ç', 'c', 'c', 'c', 'd', 'ð', 'è', 'é', 'ê', 'ë', 'e', 'e', 'g', 'g', 'h', 'î', 'ì', 'í', 'ï', 'i', 'j', 'l', 'ñ', 'n', 'n', 'ò', 'ö', 'ô', 'ó', 'õ', 'ø', 'r', 's', 's', 's', 'š', 'ß', 't', 'þ', 'ù', 'ú', 'û', 'u', 'ü', 'u', 'ý', 'z', 'z', 'ž'],
...
def trouveLangue(txt): # Essaye de déterminer la langue par le nombre de lettres et en fonction des accents
    ...
    for k in pourcentagesLettres.values(): # Pour chaque liste dans dans le dico
        for i, j in lettre_dans_txt(txt).items(): # pour chaque clé/valeur trouvé dans le dictionnaire
            findIt = pourcentagesLettres["Lettres"].index(i) # Trouve l'emplacement de la lettre dans le dico de liste 
            if compteur >= 1:
...
"""    for i, j in lettre_dans_txt(txt).items(): # pour chaque clé/valeur trouvé dans le dictionnaire
        for k in pourcentagesLettres.values(): # Pour chaque liste dans dans le dico
            findIt = k.index(i) # Trouve l'emplacement de la lettre dans le dico de liste            
 """            
def startIt():
